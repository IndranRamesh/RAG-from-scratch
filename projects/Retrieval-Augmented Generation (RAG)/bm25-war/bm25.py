...
class BM25:
    ...
    def _tokenize(self, text) -> list[Any]:
        """
        ...
        return [
            t.lower()
            for t in re.findall(pattern=r"\b[a-zA-Z]{3,}\b", string=text.lower())
        ]
    ...
    def fit(self, documents):
        try:
            ...
            self.avg_doc_len = (
                total_len / self.corpus_size if self.corpus_size > 0 else 0
            )
            ...
            for term, docs in self.term_to_docs.items():
                ...
                self.idf[term] = math.log(
                    1 + (self.corpus_size - n_t + 0.5) / (n_t + 0.5)
                )
            ...
            print(
                f"""Error in Average Doc length computation {self.avg_doc_len:.1f} & Vocabulary Size → {len(self.idf)}."""
            )
            return 0.0
...
if __name__ == "__main__":
    # Generate 1000 docs
    words = ["cat", "dog", "sat", "mat", "log", "pet", "rat", "hat", "bat", "fat"]
    large_docs = [
        {
            "title": f"Doc_{i}",
